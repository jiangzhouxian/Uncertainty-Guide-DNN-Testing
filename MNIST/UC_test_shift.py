...
parser.add_argument('alpha', help="the max perturbation", type=float, default = 2/255)
...
print("\n\n")
output_directory = './0506UC_shift/Model' + str(args.target_model + 1)+'/'+ str(args.transformation)+'/'
...
model4 = Model4(input_tensor=input_tensor)
Models = [model1,model2,model3,model4]
Models_na = ['model1','model2','model3','model4']
...
for current_seed in seed_nums:
    ...
    if args.transformation =='central_crop':
        orig_img = tf.image.central_crop(orig_img,0.6)
        orig_img = tf.image.resize_with_crop_or_pad(orig_img,28,28)
        ...
        orig_img = tf.image.resize_with_crop_or_pad(orig_img,20,20)
        orig_img = tf.image.resize_with_crop_or_pad(orig_img,28,28)
        orig_img = orig_img.numpy() 
                
    elif args.transformation =='brightness':
        orig_img =tf.image.random_brightness(orig_img,0.05)
        orig_img = orig_img.numpy()
    elif args.transformation =='contrast':
        orig_img =tf.image.random_contrast(orig_img,0.001,0.005)
        orig_img = orig_img.numpy()
    
    gen_img = orig_img.copy() 
    if args.target_model == 0:
        pre_model =  model1
        threshold_ensemble = 0.0156
        thre_PE = 0.2134
        thre_En_dp = 0.0112
    elif args.target_model == 1:
        pre_model =  model2
        threshold_ensemble = 0.0148
        thre_PE = 0.4115
        thre_En_dp = 0.0108
    elif args.target_model == 2:
        pre_model =  model3
        threshold_ensemble = 0.0155
        thre_PE = 0.2265
        thre_En_dp = 0.0107
    elif args.target_model == 3:
        pre_model =  model4
        threshold_ensemble = 0.0122
        thre_PE = 0.3935
        thre_En_dp = 0.0105
    ...
    while len(img_list)>0:
        ...
        for iters in range(3):
            ...
            if np.linalg.norm(gen_img-orig_img)>3 or iter>args.grad_iterations:
                ...
                if not pred1 == y_true:
                    #if ensemble < threshold_ensemble:
                    if Pe_dp <= thre_PE:
                        total_l2+=l2_norm
...
print('model and transformation:',args.target_model,args.transformation)
